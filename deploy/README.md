...
`endpoints.storageGraph` is optional. When it is absent, Sankey shows an unconfigured notice and does not fetch; Graph is unchanged. `endpoints.labelValues` feeds both the Graph filter bar and Sankey's single-value az/env selectors. It is optional too: without it the Graph filter bar's identity dropdowns come up empty and Sankey's az/env fall back to free-text entry, since the storage-graph endpoint requires both values.
