...
A shared `QueryButton` (`src/shared/ui/QueryButton.tsx`) takes `{ dirty, inFlight, disabled, disabledReason, onQuery, onCancel }` and renders `Query` (accented while `dirty`) or `Cancel` while `inFlight`, plus the reason text beside it when disabled. Both `FilterBar` and `SankeyScopeBar` place it as the last labelled column of their control row. The nav bar's Reload keeps its single job — re-run the applied selection — and is disabled before the first commit, while in flight, and while the Sankey draft is incomplete. `PageStatus` gains `phase: 'awaiting' | 'loading' | 'ready' | 'error' | 'cancelled'` so the status lamp and readout can say "awaiting Query" and "cancelled" without inferring them from `lastLoadedAt` / `error`.
...
`ScopeSelect` gains `allRow?: boolean` (default `true`); the root value control uses `mode="multi"`, `allRow={false}`, `allowCustom`, `emptyLabel="Pick values to add"`. `Add` calls `addRoots(kind, values[])`, which validates each `pod` value and refuses the whole Add with the inline error if any is malformed (one bad value must not silently drop while its siblings land). Root pills, the required-root text and the pod error stay on the row below the controls, as the spec's alignment rule demands.
