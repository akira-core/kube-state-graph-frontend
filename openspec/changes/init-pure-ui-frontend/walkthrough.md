...
| Root value dropdown, per kind, from the drawn body; typed value still works | `deriveSankey.test.ts` `rootValueOptions`; `SankeyScopeBar.test.tsx` (per-kind list, kind switch clears, custom value) |
