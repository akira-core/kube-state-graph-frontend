...
The input is the `/v1/storage-graph` body (see decision 17), and derivation does four things: layer by `labels.tier`, take one direction's weight from each `storage-flow` edge per the current mode, derive the `application` / `namespace` columns by walking each pod's `data.parent` and summing that pod's drawn inbound weights per direction (decision 20), sort by total flow. **No other summing and no splitting** — the backend already guarantees per-tier conservation and has already completed the RWX even split; recomputing on the frontend would only produce a set of numbers that do not match. The derived sums are of those conserved values and never touch them.

The layout has four steps, each a unit-testable pure function:

1. **Column x** — seven fixed columns (`netapp-node` / `netapp-aggr` / `netapp-svm` / `pvc` / `pod` / `application` / `namespace`; the Kubernetes node is never a column, see decision 20); column width is the widest box card in that column, column gap is fixed.
2. **Order within a column** — the spec's deterministic ordering (total flow descending, ties by `label` lexicographic order); the pod column is grouped by namespace first, then sorted; under the `Node` layout it is first partitioned into wrappers ordered by node name, each wrapper then ordered by the same rule, unscheduled pods after the last wrapper.
...
The rendering layer adds four more things: node box cards (title / divider / subtitle, kinds distinguished by solid versus dashed stroke, `namespace` as the leaf card), ribbon gradients and on-ribbon values (stroke halo, not an opaque backing plate), column headers and namespace color bars, and — under the `Node` layout — the wrapper box per Kubernetes node around its pod cards. Hover full-path highlight via class toggling, tooltip, and click-to-locate across views are all ordinary React event handling. Colors always come from decision 3's tokens.
...
### 20. Sankey derived columns and node grouping

**Decision:** the Sankey draws seven columns: the backend's five `storage-flow` tiers (`netapp-node` → `netapp-aggr` → `netapp-svm` → `pvc` → `pod`) and two **derived** columns, `application` → `namespace`, that the view builds by walking each pod's `data.parent` chain to the first ancestor of each kind and summing, per direction, that pod's drawn inbound `pvc-pod` weights. The Kubernetes `node` is never a column. A `Layout` segmented control (`Flat`, default / `Node`) decides whether pods sit loose in the pod column or inside a wrapper per Kubernetes node (the `pod-node` edge's target), wrappers ordered by node name A→Z, unscheduled pods below them. The layout is transient view state — not in the URL, not persisted — like the Graph's pod-parent mode.

**Why derived on the frontend:** the backend deliberately emits no edge above the pod, and `graph-data-source` D6 tells a consumer that wants those levels exactly what to do: walk `data.parent` and sum the conserved weights. The sum is safe precisely because its inputs are already conserved; it is the only client-side arithmetic, it reads only drawn links (so it can never exceed what is on screen), and it never writes back into a backend tier — the spec's "weights come straight from the backend" rule keeps its force for the five tiers and gains one named exception, marked "derived from member pods" wherever the value is shown.

**Why the node is a wrapper, not a column:** `pod → node` is where a pod runs, not where its bytes go next. As a sixth column its ribbons only restated the `pvc-pod` weights and made the flow look as if it continued past the pod. Nesting is what the Graph's `node` pod-parent mode does with the same hop, and it leaves the flow reading correctly: every ribbon still ends at a pod. Wrappers are ordered by name because a node is an inventory item the operator looks up by name; ordering them by flow would move a node on every refresh.

**Why transient:** the user's call, consistent with the pod-parent mode this mirrors; the URL contract (decision 18) stays "request-shaping inputs plus `mode`". The two controls share the `Layout` label and a `Node` segment but no state.

**Why a pod without an application spans straight to its namespace:** the alternative — a synthesized "(no application)" card — would draw a node the body does not hold. The spec already forbids that for the FlexGroup gap, for the same reason: a drawn card is either something the backend returned or a sum of things it returned.

**Alternatives considered:** _application / namespace as backend tiers_ — a backend change (a stated non-goal), and the backend's reason for not doing it stands: a pod need not have an application. _Node as a column under the `Node` layout_ — restates weights, see above. _`group=node` in the URL_ — would have followed `mode`; rejected in favour of the pod-parent-mode precedent.
...
- **Derived sums read like backend numbers** → the `application` / `namespace` ribbons carry values the backend never produced; an operator comparing one with a Harvest counter finds no series to match. **Mitigation:** every derived value is marked "derived from member pods" in tooltips and tables, the sum is taken only over drawn links, it is never written back into a backend tier, and the fixture test asserts `mongodb → prod` equals the sum of its two pods.
...
1. Project scaffold and toolchain → 2. Tokens and theme layer → 3. Runtime config loading and error screen → 4. Pure logic port (including its tests) → 5. App shell and routing → 6. Graph view (canvas → legend → interaction → search → detail) → 7. Graph request assembly (time range + filter bar) → 8. storage-graph source and Sankey view (estate / root selectors → tier derivation → rendering → cross-view Locate) → 9. Container and k8s manifests → 10. e2e and CI → 11. Grafana-style dropdowns, page split and URL scope (revision, tasks §24) → 12. Sankey derived columns and node grouping (revision, tasks §27).
